 ...
 * 装修app（装修案例详情）
 */
@Service("renovationCaseDetailServiceProtocol")
public class renovationCaseDetailServiceProtocol implements ProtocolUtilService {
    protected final Logger logger = LoggerFactory.getLogger(SignInServiceProtocol.class);
    private final static String interfaceDesc = "装修app（装修案例详情）";
    /**
     *
     * @param code 访问的接口编码
     * @param inputMap 请求参数
    ...
    public Map service(String code, Map inputMap) {
        ...
        try {
            ...
            logger.info("{} 装修app（装修案例详情）接口业务逻辑处理结果：{}",interfaceDesc,outMap.get("msg"));
        }
